...
const booking: Booking = { id: "BK-CMS", advertiser: "Advertiser", inventoryId: "INV-CMS", campaign: "Summer Launch", start: "2099-07-01", end: "2099-07-31", adSlots: 1, creativeStatus: "pending review", status: "creative review", spend: 1000, paid: false, pop: 42, createdBy: "USR-ADV" };
...
const media: MediaResource = { id: "MED-CMS", inventoryId: "INV-CMS", ownerId: "USR-OP", title: "Welcome Video", originalName: "welcome.mp4", mimeType: "video/mp4", mediaType: "video", approvalStatus: "approved", sizeBytes: 100, publicUrl: "/media/MED-CMS", createdAt: "2026-07-09T10:00:00.000Z" };
...
test("content library visualizes account resources and filters by status", async () => {
  ...
  await user.type(screen.getByLabelText("Search resources"), "summer");
  await user.click(screen.getByRole("button", { name: "Clear resource search" }));
  expect(screen.getByLabelText("Search resources")).toHaveFocus();
...
test("delegated device content stays visibly in review until an institution approves it", () => {
  render(<ContentLibraryView currentUser={operator} inventory={inventory} bookings={[]} creatives={[]} mediaResources={[{ ...media, approvalStatus: "pending review" }]} onDeleteMedia={vi.fn(async () => true)} onOpenCreative={vi.fn()} onOpenInventory={vi.fn()} />);
