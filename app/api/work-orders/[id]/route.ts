...
export async function PATCH(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const result = await client.query<{ placement_id: string; version: number; status: string; organization_id: string }>(`SELECT installation_work_orders.placement_id,installation_work_orders.version,installation_work_orders.status,campaigns.organization_id
    ...
    const next = action === "schedule" || action === "reschedule" ? "scheduled" : action === "start" ? "in_progress" : action === "remove" ? "removed" : "blocked";
