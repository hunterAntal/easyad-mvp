...
export async function listCampaignSummaries(user: DbUser, options: { query?: string; status?: string; sort?: "updated"|"name"|"start"; direction?: "asc"|"desc"; page?: number; pageSize?: number } = {}) {
  ...
  const result = await getDb().query<{
    ...
    COUNT(placements.id) FILTER (WHERE placements.status IN ('requested','creative_required','ready_for_fulfillment')) blocker_count,
    COALESCE(SUM(placements.estimated_media_cost + placements.estimated_production_cost + placements.estimated_installation_cost),0) estimated_total
...
export async function getCampaignDetail(user: DbUser, id: string) {
  ...
  return { campaign: campaign.rows[0], placements: placements.rows, quotes: quotes.rows, activity: activity.rows };
}

export async function createCampaignPlan(user: DbUser, input: {
  organizationId?: string; name: string; objective: string; geography: string; startDate: string; endDate: string;
  ...
  if (!input.name.trim() || !input.objective.trim() || !input.startDate || !input.endDate || input.endDate < input.startDate) throw new CampaignError(422, "Complete the campaign name, objective, and valid dates");
  ...
  try {
    ...
    }>("SELECT id,name,price,delivery_mode,production_lead_days,installation_lead_days FROM inventory WHERE id=ANY($1::text[]) AND approval_status='approved' FOR SHARE", [inventoryIds]);
    ...
    for (const unit of inventory.rows) {
      ...
      await client.query(`INSERT INTO placements (id,campaign_id,inventory_id,delivery_mode,start_date,end_date,status,estimated_media_cost,estimated_production_cost,estimated_installation_cost,price_snapshot,specification_snapshot,creative_due_at,created_at,updated_at)
        ...
        [placementId,campaignId,unit.id,unit.delivery_mode,input.startDate,input.endDate,media,production,installation,JSON.stringify({dailyRate:Number(unit.price),currency:"CAD",source:"operator_reported",capturedAt:now}),specs?.rows[0]?JSON.stringify(specs.rows[0]):null,input.creativePath==="later"?input.startDate:null,now]);
