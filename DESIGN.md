...
The authenticated shell uses a 244px desktop sidebar that collapses to a 76px icon rail. The choice is remembered rather than asked for, and a collapsed nav item keeps its label in the accessibility tree by clipping rather than removing it, so a screen reader still announces the destination. The rail does not apply below 1180px, where the sidebar is already a horizontal bar. The sidebar and a natural-height document workspace. The Civic Screen Operations variant keeps that geometry and every shared workspace component, but replaces marketplace workspace switching with a fixed institution scope card, civic masthead, and government-route navigation. Panels use a 16px gap and compact internal padding. Map and preview surfaces own their bounded aspect or scroll behavior; they must not impose viewport height or overflow constraints on sibling forms. Public sections use up to 56px inline space at wide viewports and collapse to one column below the existing 900px breakpoint. The marketplace advertises the civic product only through a compact final-page gateway; detailed institutional positioning belongs to `/government/about`, keeping the top navigation focused on marketplace actions.
