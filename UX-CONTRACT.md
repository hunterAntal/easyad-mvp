...
|---|---|---|---|---|---|---|---|
| Create device | `Create device` | stable busy button | Device/inventory workspace | `Device created` toast | inline error + retry | created device | inventory API |
...
- Booking creative requirement: every buyer booking submission includes one valid PNG or JPEG image, or an animated GIF for digital inventory (maximum 50 MB). MIME type and file signature are validated server-side. The booking and its immutable `pending review` creative record commit together before success is announced; there is no `upload later` booking path. A failed submission preserves the selected file for retry, and subsequent requested revisions remain available through the creative workspace.
